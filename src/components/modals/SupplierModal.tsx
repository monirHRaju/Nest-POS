...
import { Supplier, SupplierInput } from "@/store/api/suppliersApi";
...
export function SupplierModal({ open, item, onClose, onSubmit, loading = false }: Props) {
  const [formData, setFormData] = useState<SupplierInput>(empty);
  ...
  const set = (k: keyof SupplierInput, v: any) => setFormData({ ...formData, [k]: v });

  return (
    <Modal
      open={open}
      onClose={onClose}
      title={item ? "Edit Supplier" : "Add Supplier"}
      size="lg"
      footer={

...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const existing = await user.db.customer.findUnique({ where: { id: (await params).id } });
    if (!existing) return error("Not found", 404);
    if (existing.isWalkIn) return error("Cannot edit walk-in customer", 400);

    const updated = await user.db.customer.update({ where: { id: (await params).id }, data: data as any });
    return ok(updated);
  } catch (e) {
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const salesCount = await user.db.sale.count({ where: { customerId: (await params).id } });
    if (salesCount > 0) return error(`Cannot delete: ${salesCount} sale(s) linked`, 400);

    await user.db.customer.delete({ where: { id: (await params).id } });

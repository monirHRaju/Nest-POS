...
export async function GET(req: Request) {
  ...
  const [data, total] = await Promise.all([
    user.db.priceGroup.findMany({
      where: { name: { contains: search, mode: "insensitive" } },
      skip,
      take: pageSize,
      orderBy: { createdAt: "desc" },
    }),
    user.db.priceGroup.count({
      where: { name: { contains: search, mode: "insensitive" } },
    }),
  ]);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const existing = await user.db.priceGroup.findFirst({ where: { name } });
    if (existing) return error("Price group already exists", 400);

    const item = await user.db.priceGroup.create({

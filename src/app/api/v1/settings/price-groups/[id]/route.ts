...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  ...
  const item = await user.db.priceGroup.findUnique({ where: { id: params.id } });
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const existing = await user.db.priceGroup.findUnique({ where: { id: params.id } });
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    await user.db.priceGroup.delete({ where: { id: params.id } });

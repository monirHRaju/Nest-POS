...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (existing.status === "COMPLETED" && existing.sale) {
      await applyReturnStock(user.tenantId, params.id, existing.sale.warehouseId, -1);
    }

    await user.db.payment.deleteMany({ where: { returnId: params.id } });
    await user.db.return.delete({ where: { id: params.id } });
